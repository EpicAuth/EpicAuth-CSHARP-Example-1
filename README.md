# EpicAuth-CSHARP-Example : Please star 🌟

EpicAuth C# example SDK for https://EpicAuth.cc license key API auth.

### Tutorial Video

This video explains both how to use this example, but also how to add EpicAuth to your <ins>**OWN PROJECT**</ins> https://www.youtube.com/watch?v=5x4YkTmFH-U
...
This is common amongst all authentication systems. Program obfuscation causes false positives in virus scanners, and with the scale of EpicAuth this is perceived as a malicious domain. So, `EpicAuth.com` and `EpicAuth.win` have been blocked by many internet providers. for dashbord, reseller panel, customer panel, use `EpicAuth.cc`
...
Console.WriteLine(" Number of keys: " + EpicAuthApp.app_data.numKeys);
```
...
```cs
EpicAuthApp.check();
Console.WriteLine($" Current Session Validation Status: {EpicAuthApp.response.message}");
```
...
```cs
EpicAuthApp.log("hello I wanted to log this");
```
...
```cs
EpicAuthApp.ban("You've been banned for a reason.");
```
...
```cs
byte[] result = EpicAuthApp.download("385624");
if (!EpicAuthApp.response.success)
{
    Console.WriteLine("\n Status: " + EpicAuthApp.response.message);
    Thread.Sleep(2500);
...
This is a modified version of KeyAuth  
Original Source: https://github.com/KeyAuth  
Modifications by: EpicAuth (https://github.com/EpicAuth)
